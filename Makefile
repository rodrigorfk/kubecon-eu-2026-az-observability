...
create: preflight ## Create the Kind cluster
	kind create cluster --name $(CLUSTER_NAME) --config $(KIND_CONFIG)
	@echo "Spreading CoreDNS across zones..."
	kubectl --context kind-$(CLUSTER_NAME) patch deployment coredns -n kube-system --type merge -p '{ \
		"spec": { "template": { "spec": { "topologySpreadConstraints": [ \
			{ "maxSkew": 1, "topologyKey": "topology.kubernetes.io/zone", \
			  "whenUnsatisfiable": "DoNotSchedule", \
			  "labelSelector": { "matchLabels": { "k8s-app": "kube-dns" } } } \
		] } } } }'
	kubectl --context kind-$(CLUSTER_NAME) rollout status deployment/coredns -n kube-system --timeout=60s
